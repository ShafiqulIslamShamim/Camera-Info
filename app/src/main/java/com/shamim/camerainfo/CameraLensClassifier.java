...
public class CameraLensClassifier {
    ...
    public static Map<String, LensResult> detectLensesAndReturnMap(List<String> validCameraIds) {
        ...
        try {
            ...
            for (String id : validCameraIds) {
                ...
                if (focalLengths == null || focalLengths.length == 0 || sensorSize == null || pixelArray == null) continue;
                ...
                info.angleOfView = Camera2ApiKeysInfo.calculateAngleOfView(repFocal, sensorSize, pixelArray).floatValue();
    ...
    /**
     * Detect logical camera via API flag (physical camera ids),
     * bit test on cameraId (parse as integer), or equality of key CameraCharacteristics fields.
     */
    ...
    /**
     * Robust equality check on a small set of CameraCharacteristics properties that commonly indicate identical sensors.
     * This avoids relying on CameraCharacteristics.equals() implementation.
     */
    private static boolean characteristicsEquivalent(CameraCharacteristics a, CameraCharacteristics b) {
    ...
    /**
     * Core classification that follows the CameraIdentifier flow you provided.
     * Preserves the input order so the first camera in the list becomes MAIN.
     */
    private static void classifyAndComputeZoom(List<LensInfo> group, String label) {
        ...
        //    - getPhysicalCameraIds() non-empty OR bit-6 set on id OR characteristics equal to another camera
        ...
        for (LensInfo info : group) {
            Log.d("LensClassifier", "ID: " + info.cameraId
                    + " | EqFocal: " + String.format("%.2f", info.eqFocal) + "mm"
                    + " | AoV: " + String.format("%.1f", info.angleOfView) + "°"
                    + " | Type: " + info.type
                    + " | Zoom: " + String.format("%.2fx", info.zoomFactor)
                    + " | AE: " + info.hasAeModes
                    + " | Flash: " + info.flashSupported);
        }
    }
}