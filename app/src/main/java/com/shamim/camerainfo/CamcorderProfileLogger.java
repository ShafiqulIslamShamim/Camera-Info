...
public class CamcorderProfileLogger {
        ...
        for (int i2 : getAllQualityValues()) {
            try {
                if (CamcorderProfile.hasProfile(i, i2)) {
                    ...
                    try {
                        sb.append("Video Frame Height: ").append(camcorderProfile.videoFrameHeight).append("\n");
                    } catch (Exception e2) {
                    ...
                    arrayList.add("No CamcorderProfile available for quality: " + getQualityName(i2) + " (" + i2 + ")\n");
                }
            } catch (Exception e11) {
                arrayList.add("Error accessing profile for quality: " + getQualityName(i2) + " (" + i2 + ") - " + e11.getMessage());
            }
        }
        return arrayList;
    }
    ...
    private static String getQualityName(int i) {
        for (Field field : CamcorderProfile.class.getFields()) {
            try {
                if (Modifier.isStatic(field.getModifiers()) && field.getType() == Integer.TYPE && ((Integer) field.get(null)).intValue() == i) {
