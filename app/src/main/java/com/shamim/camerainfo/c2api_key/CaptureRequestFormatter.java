...
public class CaptureRequestFormatter {
  ...
  /**
   * Translates and lists available request keys for captured requests.
   *
   * @param cameraCharacteristics Hardware specifications object.
   * @return Formatted request keys log.
   */
  ...
  /** Helper that formats a request key name by appending descriptions and values. */
  ...
  /** Resolves a user-friendly description of the CaptureRequest key. */
  private static String getDescription(String name) {
    switch (name) {
      ...
      case "android.blackLevel.lock":
        return "Indicates whether the camera device's black level offset is locked for consistency";
      case "android.jpeg.gpsLocation":
        return "Specifies GPS location metadata to embed in the captured JPEG image";
      case "android.scaler.cropRegion":
        return "Defines the active pixel area of the sensor to be used for scaling and output";
      case "android.tonemap.curve":
        return "Specifies the complete tonemap curve used for color and brightness mapping";
  ...
  /** Resolves supported state labels for specific control requests. */
  ...
  /** Formats key-value pairs into brace-wrapped listings of descriptions. */
  ...
  /** Maps an array of primitive integer states to a formatted value listing. */
